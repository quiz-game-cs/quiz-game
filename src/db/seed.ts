...
async function seed() {
  ...
  for (let i = 0; i < CATEGORIES.length; i++) {
    ...
    const [inserted] = await db
      ...
      .values({
        code: cat.code,
        majorName: cat.majorName,
        minorName: cat.minorName,
        displayOrder: i + 1,
      })
      .returning();
    codeToId.set(cat.code, inserted.id);
  }
  console.log(`  카테고리 ${CATEGORIES.length}개 추가됨`);

  console.log("문제 시드 삽입...");
  for (const q of SEED_QUESTIONS) {
    const categoryId = codeToId.get(q.categoryCode) ?? null;
    if (!categoryId) {
      console.warn(`  경고: code ${q.categoryCode} 매칭 실패. 9999로 fallback.`);
    }
    const [inserted] = await db
      .insert(questions)
      .values({
        text: q.text,
        answers: q.answers,
        categoryId: categoryId ?? codeToId.get("9999")!,
  ...
  console.log(
    `\n완료! 카테고리 ${CATEGORIES.length}개, 문제 ${SEED_QUESTIONS.length}개, 고스트 ${SEED_QUESTIONS.length * 3}개 삽입.`
  );
}
